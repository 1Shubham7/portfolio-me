...
Linux filling your RAM is a feature. An empty RAM graph means Linux isn't doing its job.

---

## Quick Reference

| Metric | Source | What it includes | Should you panic? |
|---|---|---|---|
| Node Exporter "used" | `/proc/meminfo` | Apps + all caches + kernel | No |
| kube-state-metrics RSS | cAdvisor → kubelet → k8s API | Only app workloads | If it's high, investigate |
| MemAvailable | `/proc/meminfo` | Real remaining headroom | If it's near zero, yes |

---

The node had 24 GB available the whole time. Zero issue. The only thing that was broken was which graph I was trusting.